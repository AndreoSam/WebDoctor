...
// Vercel's deployed application bundle is read-only. Use /tmp for best-effort
// runtime history there; local development keeps the repository data file.
const HISTORY_FILE=process.env.VERCEL
  ? '/tmp/webdoctor-capacity-history.json'
  : fileURLToPath(new URL('../data/capacity-history.json',import.meta.url));
...
export async function runSteppedCapacityLab(verifiedOrigin,{pageUrl,save=true}={}){
...
export async function runJourneyCapacityLab(verifiedOrigin,{pageUrls=[],users=5,durationSeconds=10,save=true}={}){
  users=Number(users); durationSeconds=Number(durationSeconds);
  ...
  for(const href of targets){
    ...
    try{const r=await fetch(href,{method:'GET',redirect:'follow',signal:AbortSignal.timeout(10000),headers:{'user-agent':'WebDoctor-JourneyLoad/1.8 Preflight'}});await r.arrayBuffer();row={...row,finalUrl:r.url,status:r.status,httpOk:r.ok,baselineMs:Math.round(performance.now()-t0),reason:r.ok?'Ready':`HTTP ${r.status}`};}catch(e){row.reason=e?.name==='TimeoutError'?'Timed out':'Request failed';}
    if(browser){const page=await browser.newPage({viewport:{width:1280,height:800}});try{await page.goto(href,{waitUntil:'domcontentloaded',timeout:12000});await page.waitForTimeout(900);const diag=await page.evaluate(()=>{const visible=el=>{const s=getComputedStyle(el),r=el.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>0&&r.height>0};const interactive=[...document.querySelectorAll('input,textarea,select,button,a[href],[role=button],[role=link]')].filter(visible).length;const body=(document.body?.innerText||'').trim();const errorLike=/\b(404|not found|page not found|something went wrong|application error)\b/i.test(body.slice(0,3000));return {interactive,bodyLength:body.length,errorLike,title:document.title||''};});row.interactiveCount=diag.interactive;row.browserRendered=diag.bodyLength>20&&!diag.errorLike;row.browserState=diag.errorLike?'ERROR UI':row.browserRendered?(diag.interactive?'INTERACTIVE':'RENDERED'):'EMPTY/LOADING';}catch{row.browserState='BROWSER FAILED';}finally{await page.close().catch(()=>{});}}
    const finalSameOrigin=(()=>{try{return new URL(row.finalUrl).origin===origin}catch{return false}})();row.ok=Boolean(row.httpOk&&finalSameOrigin);if(!row.ok&&row.browserRendered)row.reason=`SPA route: direct HTTP ${row.status??'failed'}, browser ${row.browserState.toLowerCase()}`;preflight.push(row);
  }
  if(browser)await browser.close().catch(()=>{});
  const valid=preflight.filter(x=>x.ok),invalid=preflight.filter(x=>!x.ok);
  if(valid.length<2){const result={origin,mode:'journey',users,durationRequested:durationSeconds,requests:0,successful:0,errors:0,errorRate:0,avgMs:0,p95Ms:0,requestsPerSecond:0,pages:[],preflight,routeHealth:invalid.length?'NEEDS ATTENTION':'PASS',verdict:'NOT DETERMINED',loadVerdict:'NOT TESTED',aborted:false,abortReason:'',notice:'Journey load was not started because fewer than 2 selected routes passed pre-flight.',scope:'Browser-aware pre-flight distinguishes direct HTTP failures, SPA-rendered routes and invalid routes. Only successful direct GET routes are eligible for this read-only HTTP load test.',limits:'5 routes max · 25 workers max · 50 requests/sec global cap · 2,000 requests max · auto-stop above 30% load errors.',createdAt:new Date().toISOString()};if(save)await saveHistory(result);return result;}
  const loadTargets=valid.map(x=>x.url),baselines=Object.fromEntries(valid.map(x=>[x.url,x.baselineMs]));
  const started=Date.now(),deadline=started+durationSeconds*1000,latencies=[];let requests=0,ok=0,errors=0,active=true,aborted=false,abortReason='';const perPage=Object.fromEntries(loadTargets.map(x=>[x,{requests:0,ok:0,errors:0,latencies:[],statusCounts:{}}]));let nextPermit=performance.now();const intervalMs=1000/50;let gate=Promise.resolve();
  async function rateGate(){let release;const prev=gate;gate=new Promise(r=>release=r);await prev;const now=performance.now(),wait=Math.max(0,nextPermit-now);if(wait)await sleep(wait);nextPermit=Math.max(nextPermit,performance.now())+intervalMs;release();}
  ...
  const elapsed=(Date.now()-started)/1000,errorRate=requests?errors/requests*100:0,avg=latencies.length?latencies.reduce((a,b)=>a+b,0)/latencies.length:0,p95=Math.round(percentile(latencies,95));let loadVerdict='PASS';if(aborted||errorRate>=10)loadVerdict='FAIL';else if(errorRate>1||p95>2000)loadVerdict='NEEDS ATTENTION';
